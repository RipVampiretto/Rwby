...
async function getStats(userId, guildId) {
    ...
    let stats = await db.queryOne(
        'SELECT * FROM user_active_stats WHERE user_id = $1 AND guild_id = $2',
        [userId, guildId]
    );
    if (!stats) {
        stats = {
...
async function updateStats(stats) {
    ...
    `, [stats.user_id, stats.guild_id, stats.msg_count_60s, stats.msg_count_10s, stats.last_msg_content, stats.last_msg_ts || new Date().toISOString(), stats.duplicate_count]);
}
