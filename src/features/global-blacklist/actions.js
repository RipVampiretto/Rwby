...
async function flushBanQueue(logChannelId) {
    ...
    try {
        ...
        logger.info(`[global-blacklist] Sent aggregated notification to ${logChannelId}: ${bans.length} users`);
    } catch (e) {
        logger.error(`[global-blacklist] Failed to send aggregated notification: ${e.message}`);
    }
...
async function processNewCasBans(newUsers) {
    ...
    logger.info(`[global-blacklist] Global bans executed: ${globalBanCount}, failed: ${failedBans}`);
...
async function notifyParliament(newUsers, banCount, guildCount) {
    ...
    try {
        ...
        logger.info('[global-blacklist] Parliament notification sent');
    } catch (e) {
        logger.error(`[global-blacklist] Failed to notify Parliament: ${e.message}`);
    }
}
