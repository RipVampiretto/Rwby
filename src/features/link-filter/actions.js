...
async function executeAction(ctx, verdict) {
    ...
    if (type === 'blacklist') {
        ...
        // Log only if enabled
        if (logEvents['link_delete'] && actionLog.getLogEvent()) {
        ...
        logger.info(`[link-filter] Deleted blacklisted link from ${user.id}: ${domain}`);
        ...
        logger.debug(`[link-filter] Unknown domain forwarded to Parliament: ${domain}`);
    }
}
