...
async function processMedia(ctx, config) {
    ...
    logger.info(`[media-filter] 🔄 Starting processMedia - Chat: ${chatId}, User: ${userId}`);
    ...
    if (ctx.message.photo) {
        ...
        logger.debug(
            `[media-filter] 📷 Photo detected - Size: ${fileSize} bytes, Dimensions: ${photo.width}x${photo.height}`
        );
        ...
        logger.debug(
            `[media-filter] 🎥 Video detected - Size: ${fileSize} bytes, Duration: ${ctx.message.video.duration}s`
        );
    } else if (ctx.message.animation) {
        fileId = ctx.message.animation.file_id;
        type = 'gif';
        fileSize = ctx.message.animation.file_size || 0;
        logger.debug(`[media-filter] 🎞️ Animation/GIF detected - Size: ${fileSize} bytes`);
    } else if (ctx.message.document) {
        fileId = ctx.message.document.file_id;
        fileSize = ctx.message.document.file_size || 0;
        logger.debug(
            `[media-filter] 📄 Document detected - MIME: ${ctx.message.document.mime_type}, Size: ${fileSize} bytes`
        );
        if (ctx.message.document.mime_type?.startsWith('video')) type = 'video';
            ...
            logger.debug(`[media-filter] ⏭️ Document is not image/video, skipping`);
            return;
        }
        ...
        logger.debug(`[media-filter] 🪙 Sticker detected - Size: ${fileSize} bytes, is_video: ${sticker.is_video}`);
        type = sticker.is_video ? 'gif' : 'photo';
    }

    logger.info(`[media-filter] 📁 Getting file info from Telegram - FileId: ${fileId?.substring(0, 20)}...`);
    ...
    if (fileSize > MAX_FILE_SIZE) {
        logger.warn(
            `[media-filter] ⚠️ File too large (${(fileSize / 1024 / 1024).toFixed(2)} MB), limit is ${(MAX_FILE_SIZE / 1024 / 1024).toFixed(0)} MB. Skipping analysis.`
        );
        return;
    }
    ...
    if (type === 'video' && ctx.message.video?.duration > MAX_VIDEO_DURATION) {
        logger.warn(
            `[media-filter] ⚠️ Video too long (${(ctx.message.video.duration / 60).toFixed(1)} min), skipping analysis`
        );
        return;
    }
    ...
    try {
        ...
        if (err.description && err.description.includes('file is too big')) {
            logger.warn(
                `[media-filter] ⚠️ Telegram API Error: File is too big to download via Cloud API. Consider using Local API Server.`
            );
            return;
        }
        // Re-throw other errors or log and return
        logger.error(`[media-filter] ❌ Error getting file info: ${err.message}`);
        return;
    }

    logger.debug(`[media-filter] 📁 File path: ${file.file_path}`);
    ...
    if (IS_LOCAL_API) {
        ...
        if (file.file_path.startsWith('/var/lib/telegram-bot-api/')) {
            ...
            if (fs.existsSync(mappedPath)) {
                ...
                logger.debug(`[media-filter] 📂 Local API detected, found direct file at: ${mappedPath}`);
            }
        }
    ...
    try {
        if (directLocalPath) {
            logger.info(`[media-filter] 📂 Copying file from local storage: ${directLocalPath} -> ${localPath}`);
            fs.copyFileSync(directLocalPath, localPath);
        } else {
            logger.info(
                `[media-filter] ⬇️ Downloading file to: ${localPath} (Source: ${IS_LOCAL_API ? 'Local API' : 'Cloud API'})`
            );
            logger.info(`[media-filter] 🔗 Download URL: ${downloadUrl}`);
            await downloadFile(downloadUrl, localPath);
        }
    } catch (e) {
        logger.error(`[media-filter] ❌ Download/Copy failed: ${e.message}`);
        return false;
    }
    const downloadTime = Date.now() - downloadStart;

    const actualSize = fs.statSync(localPath).size;
    logger.info(`[media-filter] ✅ File ready - Size: ${actualSize} bytes, Time: ${downloadTime}ms`);
    ...
    try {
        ...
        if (caption) {
            logger.debug(`[media-filter] 📝 Caption present: "${caption.substring(0, 50)}..."`);
        }

        if (type === 'video' || type === 'gif') {
            logger.info(`[media-filter] 🎬 Starting VIDEO/GIF analysis...`);
            isNsfw = await checkVideo(localPath, config, reasons, caption, chatId);
        } else {
            logger.info(`[media-filter] 🖼️ Starting IMAGE analysis...`);
...
async function downloadFile(url, dest) {
    ...
    return new Promise((resolve, reject) => {
        ...
        client
            .get(url, response => {
                logger.debug(`[media-filter] ⬇️ downloadFile: Got response, status: ${response.statusCode}`);
                response.pipe(file);
                file.on('finish', () => {
                    logger.debug(`[media-filter] ⬇️ downloadFile: File write finished`);
                    file.close(resolve);
                });
            })
            .on('error', err => {
...
async function checkImage(imagePath, config, reasons, caption = null, chatId = null) {
    ...
    logger.debug(`[media-filter] 🖼️ checkImage: Base64 size: ${base64Size}KB`);

    logger.info(`[media-filter] 🤖 Sending image to Vision LLM for analysis...`);
    const llmStart = Date.now();
    const res = await callVisionLLM(base64, caption, chatId);
    const llmTime = Date.now() - llmStart;

    logger.info(
        `[media-filter] 🤖 LLM Response (${llmTime}ms): primary=${res.primary_category}, uncertainty=${res.uncertainty}, reason=${res.reason || 'N/A'}`
    );
...
async function checkVideo(videoPath, config, reasons, caption = null, chatId = null) {
    ...
    // Get duration
    logger.debug(`[media-filter] 🎬 Getting video duration...`);
    const duration = await getVideoDuration(videoPath);
    if (!duration) {
        logger.warn(`[media-filter] ⚠️ Could not get video duration, skipping analysis`);
        return false;
    }
    logger.info(`[media-filter] 🎬 Video duration: ${duration.toFixed(2)}s`);

    // LIMIT: Skip videos longer than 5 minutes
    const MAX_DURATION = 300; // 5 minutes in seconds
    if (duration > MAX_DURATION) {
        logger.warn(`[media-filter] ⚠️ Video too long (${(duration / 60).toFixed(1)} min > 5 min), skipping analysis`);
        return false;
    }
    ...
    logger.info(`[media-filter] 🎬 Duration: ${duration.toFixed(1)}s → Extracting ${actualFrames} frames uniformly`);
    ...
    logger.info(`[media-filter] ⚡ Extracting ${timestamps.length} frames in parallel...`);
    ...
    logger.info(
        `[media-filter] ⚡ Extraction complete: ${validFrames.length}/${timestamps.length} frames in ${Date.now() - extractStart}ms`
    );

    if (validFrames.length === 0) {
        logger.warn(`[media-filter] ⚠️ No frames extracted successfully`);
        return false;
    }

    logger.info(`[media-filter] 🎬 Analyzing ${validFrames.length} frames individually (frame-by-frame)...`);

    try {
        // Analyze each frame individually (unified logic with images)
        for (let i = 0; i < validFrames.length; i++) {
            const frame = validFrames[i];
            logger.debug(`[media-filter] 🤖 Frame ${i + 1}/${validFrames.length} at ${frame.timestamp.toFixed(1)}s`);
            ...
            if (isNsfw) {
                ...
                logger.warn(
                    `[media-filter] 🚨 NSFW detected at frame ${i + 1} (${frame.timestamp.toFixed(1)}s): ${frameReasons[0]}`
                );
                return true;
            }
        }

        logger.info(`[media-filter] ✅ All ${validFrames.length} frames passed check`);
        return false;
    } finally {
        // Cleanup all frame files
        logger.debug(`[media-filter] 🧹 Cleaning up ${validFrames.length} frame files...`);
...
function getVideoDuration(filePath) {
    return new Promise(resolve => {
        ...
        fluentFfmpeg.ffprobe(filePath, (err, metadata) => {
            if (err) {
                logger.error(`[media-filter] ❌ ffprobe error: ${err.message}`);
                resolve(0);
            } else {
                logger.debug(
                    `[media-filter] 🎬 ffprobe: Duration = ${metadata.format.duration}s, Format = ${metadata.format.format_name}`
                );
...
function extractFrame(videoPath, timestamp, outputPath) {
    return new Promise((resolve, reject) => {
        ...
        fluentFfmpeg(videoPath)
            ...
            .output(outputPath)
            .on('end', () => {
                logger.debug(`[media-filter] 🎬 ffmpeg: Frame extracted successfully`);
                resolve();
            })
            .on('error', err => {
                logger.error(`[media-filter] ❌ ffmpeg error: ${err.message}`);
                reject(err);
            })
            .run();
    });
...
- Be conservative, consistent, and honest about uncertainty.`;
    ...
    try {
        ...
        if (!result.scores || typeof result.scores !== 'object') {
            logger.warn(`[media-filter] Invalid response structure, creating safe default`);
            result = {
                scores: { safe: 1.0 },
        ...
        logger.debug(`[media-filter] 🤖 LLM parsed result: ${JSON.stringify(result)}`);
...
async function analyzeMediaOnly(ctx, config) {
    ...
    try {
        ...
        logger.error(`[media-filter] analyzeMediaOnly: getFile error - ${e.message}`);
        return { isNsfw: false };
    }
    ...
    try {
        ...
        logger.error(`[media-filter] analyzeMediaOnly error: ${e.message}`);
        return { isNsfw: false };
    } finally {
        try {
            fs.unlinkSync(localPath);
